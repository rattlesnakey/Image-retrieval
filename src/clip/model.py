...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 text_smoothing: str, #! text smoothing
    ...
    def encode_text(self, text, text_smoothing=False):
        ...
        if text_smoothing:
            ...
            #! one-hot
            word_embeddings = self.bert.get_embedding_matrix()
            one_hot = torch.zeros_like(input_probs).scatter_(2, text.reshape(text.shape[0], text.shape[1], 1).long(), 1.0).to(x.device)
            now_probs = 0.5 * (F.softmax(input_probs / 1, dim=-1).to(x.device)) + (1 - 0.5) * one_hot # 4 2 0.5 0.25
            x_enhanced =  now_probs @ word_embeddings.weight
            return x_enhanced
        else:
            x = self.bert(text, attention_mask=attn_mask)[0][0].type(self.dtype) 
            # x = self.bert(text, attention_mask=attn_mask)[0].type(self.dtype)  #! output_sequence
            return x[:, 0, :] @ self.text_projection
            # return x


    
    def forward(self, image, text, text_smoothing=False):
        ...
        text_features = self.encode_text(text, text_smoothing)
